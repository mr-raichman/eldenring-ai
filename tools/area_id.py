...
from eldenring_ai.config import BEFORE_MARGIT_AREA_ID, MARGIT_AREA_ID
...
def _label(area_id):
    if area_id == MARGIT_AREA_ID:
        return "Margit arena"
    if area_id == BEFORE_MARGIT_AREA_ID:
        return "pre-Margit grace"
    if area_id == 0:
...
def main():
    ...
    print("Live monitor of area ID.")
    print("Cross the fog gate in both directions and watch the value.")
    print(f"MARGIT_AREA_ID = {MARGIT_AREA_ID}, "
          f"BEFORE_MARGIT_AREA_ID = {BEFORE_MARGIT_AREA_ID}. Ctrl+C to stop.\n")
